...
# HARDENING Progress Update — HARDENING-10 Provider Callback / Payment Callback / PayTR Hattı

## 1. Kayıt Kapsamı

Bu kayıt, HARDENING progress record dosyasındaki önceki kayıtlardan sonra yürütülen HARDENING-10 provider callback ve payment callback çalışmalarını kapsar.

Kapsanan ana hat:

- HARDENING-10A — Generic provider callback foundation
- HARDENING-10B — Common provider callback ingestion / security boundary
- HARDENING-10C — PayTR-first payment callback normalization, BFF bridge, worker, lookup ve payment owner transition hattı

Bu kayıt implementation dosyası değildir. Yapılan işleri, kararları, doğrulama sonuçlarını, kalan limitation’ları ve sonraki adımları merkezi olarak tutar.

---

## 2. HARDENING-10 Genel Karar

HARDENING-10 hattında temel hedef, dış provider callback/webhook girişlerinin platform domain truth’unu doğrudan mutate etmesini engelleyerek, önce güvenli ingestion + normalization + verification + worker handoff hattı kurmaktı.

Temel mimari kararlar:

- Provider callback business truth değildir.
- BFF truth owner değildir.
- Provider callback record yalnız ingestion/audit/processing record seviyesindedir.
- Payment truth sadece payment owner domain içinde değiştirilebilir.
- PayTR callback sonucu doğrudan order oluşturmaz.
- Payment success settlement/hakediş/payout anlamına gelmez.
- Order, finance, risk ve reconciliation ayrı paketlere bırakılmıştır.
- PayTR primary provider, iyzico secondary-ready provider olarak konumlandırılmıştır.
- Tek route + provider policy yaklaşımı seçilmiştir:
  - `POST /provider-callback/payment/paytr`
  - `POST /provider-callback/payment/iyzico`

---

## 3. HARDENING-10A — Provider Callback Foundation

### Durum

HARDENING-10A hattı generic provider callback contract ve persistence temelini kurdu.

### Tamamlanan paketler

| Paket | Durum | Özet |
|---|---|---|
| HARDENING-10A1 | PASS | `ProviderCallbackRecord` contract foundation eklendi. |
| HARDENING-10A2 | PASS | Provider callback signature input/result/helper foundation eklendi. |
| HARDENING-10A3 | PASS WITH LIMITATION | Provider callback persistence ve migration foundation oluşturuldu. |
| HARDENING-10A4 | PASS WITH LIMITATION | In-memory provider callback foundation smoke eklendi. |
| HARDENING-10A5 | FAIL | İlk final regression BFF runtime çalışmadığı için fail oldu. |
| HARDENING-10A5-R | PASS WITH LIMITATION | BFF runtime doğru başlatıldı; provider callback regression smoke’ları PASS oldu. |

### Yapılanlar

- Generic provider callback contract eklendi.
- Signature verification result modeli eklendi.
- Callback record persistence foundation kuruldu.
- In-memory callback repository smoke ile doğrulandı.
- Provider boundary smoke regression doğrulandı.
- İlk 10A5 failure sebebi runtime ön koşul eksikliği olarak ayrıştırıldı.
- BFF doğru env/port ile başlatılarak 10A5-R remediation yapıldı.

### Kalan limitler

- BFF callback endpoint o aşamada yoktu.
- Domain callback processing yoktu.
- Real crypto verification yoktu.
- Worker/queue/reconciliation yoktu.
- Provider-specific mapping yoktu.

---

## 4. HARDENING-10B — Common Provider Callback Ingestion / Security Boundary

### Durum

HARDENING-10B hattı generic callback ingestion route’unu, security guard’ları ve boundary’leri kurdu.

### Tamamlanan paketler

| Paket | Durum | Özet |
|---|---|---|
| HARDENING-10B-00B | INVENTORY COMPLETE | Payment callback domain inventory oluşturuldu. |
| HARDENING-10B-00C | INVENTORY COMPLETE | Shipment callback inventory oluşturuldu. |
| HARDENING-10B-00D | INVENTORY COMPLETE | Notification delivery callback inventory oluşturuldu. |
| HARDENING-10B-00E | INVENTORY COMPLETE | Payout callback inventory oluşturuldu. |
| HARDENING-10B1 | PASS | Postgres callback idempotency remediation tamamlandı. |
| HARDENING-10B2A | PASS WITH LIMITATION | Common provider callback BFF ingestion endpoint eklendi. |
| HARDENING-10B2B | PASS WITH LIMITATION | Signature guard foundation eklendi. |
| HARDENING-10B2C | PASS WITH LIMITATION | Replay / duplicate / identity conflict guard eklendi. |
| HARDENING-10B2D | PASS WITH LIMITATION | Timestamp freshness guard foundation eklendi. |
| HARDENING-10B2E | PASS WITH LIMITATION | Process-local rate/abuse guard eklendi. |
| HARDENING-10B2F | CLOSURE COMPLETE | Common ingestion security boundary final closure oluşturuldu. |

### Yapılanlar

- Generic endpoint kuruldu:
  - `POST /provider-callback/:providerDomain/:providerName`
- Provider callback ingestion persistence’e bağlandı.
- Duplicate `providerEventId` ve `idempotencyKey` davranışları Postgres tarafında in-memory davranışla hizalandı.
- Signature guard foundation eklendi.
- Replay/duplicate/identity conflict guard eklendi.
- Timestamp freshness guard eklendi.
- Nonce header okunmaya başlandı; reuse/cache ileriki pakete bırakıldı.
- Process-local rate limit eklendi:
  - 60 saniyede aynı `providerDomain + providerName + clientKey` için 20 istekten sonrası `429 PROVIDER_CALLBACK_RATE_LIMITED`
- PayTR özel ACK henüz 10B hattında yapılmadı.
- Domain mutation yapılmadı.

### Kalan limitler

- Distributed rate limit yok.
- Redis/API gateway/WAF yok.
- Provider-specific config yoktu.
- Domain processing yoktu.
- Reconciliation runtime yoktu.
- Worker/queue yoktu.

---

## 5. HARDENING-10C — PayTR-first Payment Callback Hattı

## 5.1 HARDENING-10C0 — Provider-specific Callback Mapping Inventory / Payment First Candidate
...
Payment callback mapping için provider-first envanter çıkarıldı. PayTR primary candidate, iyzico secondary-ready olarak konumlandırıldı.

Kararlar
İlk provider-specific mapping adayı PayTR seçildi.
iyzico ileride geçiş / fallback sağlayıcı olarak hazır tutulacak.
Payment callback mapping başlamadan önce normalized candidate modeli kurulmalı kararı verildi.
Domain processing bu aşamada başlatılmadı.
5.2 HARDENING-10C1 — Normalized Payment Callback Candidate Contract / Helper
Durum
PASS
Yapılanlar
NormalizedPaymentCallbackCandidate modeli eklendi.
PaymentCallbackOwnerCommandCandidate modeli eklendi.
decidePaymentCallbackCandidate helper’ı eklendi.
createNormalizedPaymentCallbackCandidate helper’ı eklendi.
Candidate business truth değildir; owner transition adayıdır.
Boundary flags false kaldı.
10C1-R Verification

İlk 10C1 kapanışında verification seti eksikti. 10C1-R ile tamamlandı.
...
Nihai karar
HARDENING-10C1 — PASS
5.3 HARDENING-10C2 — PayTR Callback Mapping Foundation Smoke
Durum
PASS WITH LIMITATION
Yapılanlar
PayTR raw callback payload modeli eklendi.
PayTR hash helper eklendi.
PayTR hash verification helper eklendi.
PayTR callback payload → normalized payment callback candidate mapping helper eklendi.
PayTR TL → TRY currency normalization kararı uygulandı.
total_amount > payment_amount tek başına mismatch sayılmadı.
Bad hash durumunda:
normalizedStatus=signature_failed
shouldProcess=false
shouldReject=true
PAYTR_HASH_FAILED risk flag
Success/failed/missing reference/unsupported/amount mismatch/currency mismatch smoke ile doğrulandı.
PayTR hash kuralı
base64(hmac_sha256(merchant_oid + merchant_salt + status + total_amount, merchant_key))
Kalan limitler
Gerçek merchant key/salt/env yoktu.
BFF live path’e bağlanmamıştı.
Payment owner mutation yoktu.
Worker/queue/reconciliation yoktu.
5.4 HARDENING-10C3 — Payment Provider Callback Response Policy Bridge / No Domain Mutation
Durum
PASS WITH LIMITATION
Yapılanlar
Tek ortak route korundu:
POST /provider-callback/:providerDomain/:providerName
PayTR için ACK response policy eklendi:
HTTP 200
Content-Type: text/plain
Body: OK
Non-PayTR provider’larda JSON envelope davranışı korundu.
Rate limit durumunda PayTR’ye OK dönülmemesi kararı uygulandı.
PayTR merchant_oid, duplicate-safe identity/reference hattına bridge edildi.
Domain mutation yapılmadı.
Kalan limitler
PayTR mapping helper live BFF path’e bağlanmamıştı.
Real config/secret yoktu.
Payment owner transition yoktu.
iyzico implementation yoktu.
5.5 HARDENING-10C4 — Payment Provider Config / Secret Resolution Foundation
Durum
PASS WITH LIMITATION
Yapılanlar
Payment provider config resolver eklendi.
PayTR primary config shape kuruldu:
PAYTR_MERCHANT_ID
PAYTR_MERCHANT_KEY
PAYTR_MERCHANT_SALT
iyzico secondary-ready config shape kuruldu:
IYZICO_API_KEY
IYZICO_SECRET_KEY
PAYMENT_PROVIDER_NAME
PAYMENT_PROVIDER_MODE
Unsupported provider/mode için sessiz fallback yapılmadı.
Secret redaction helper/smoke ile secret değerlerinin sızmadığı doğrulandı.
getPaymentProviderAdapter() resolver’a bağlandı.
PayTR/iyzico aktif seçilirse live network çağrısı yapılmadan rejected provider envelope döner.
Kalan limitler
Real secret manager yok.
PayTR live initiate yok.
iyzico implementation yok.
Callback mapping live path’e henüz bağlanmamıştı.
5.6 HARDENING-10C5 — PayTR Callback Mapping Live BFF Path / No Domain Mutation
Durum
PASS WITH LIMITATION
Yapılanlar
PayTR callback live BFF ingestion path’e normalizedPayload bridge eklendi.
PayTR config resolver + PayTR mapping helper birlikte kullanıldı.
Valid PayTR callback:
HTTP 200 text/plain OK
...
shouldReject=true
PAYTR_HASH_FAILED
Duplicate callback existing record’u korudu; normalizedPayload overwrite edilmedi.
Non-PayTR provider JSON envelope davranışı korundu.
Kalan limitler
Candidate içindeki callbackRecordId insert öncesi pending_insert placeholder idi.
PaymentAttempt lookup yoktu.
expectedAmount/expectedCurrency live lookup yoktu.
Payment owner mutation yoktu.
5.7 HARDENING-10C6 — Payment Callback Processing Worker / Owner Command Handoff Inventory
Durum
PAYMENT CALLBACK WORKER / OWNER HANDOFF INVENTORY COMPLETE / IMPLEMENTATION REQUIRED
Yapılanlar
Payment callback worker/owner handoff modeli için inventory çıkarıldı.
DB polling worker modeli ilk implementation için doğru seçenek olarak belirlendi.
BFF synchronous processing reddedildi.
Payment state model gap tespit edildi:
PENDING yok
UNKNOWN_RESULT yok
attempt FAILED/CANCELLED/EXPIRED yok
callback processed marker yok
merchant_oid / paymentAttemptId / providerReference belirsizliği dürüstçe ayrıştırıldı.
Order boundary korundu.
Finance/settlement boundary korundu.
Risk sadece candidate/advisory seviyesinde bırakıldı.
Sonraki öneri

10C7 state model / owner command contract foundation olarak belirlendi.

5.8 HARDENING-10C7 — Payment Callback State Model / Owner Command Contract Foundation
Durum
PASS WITH LIMITATION
Yapılanlar
PaymentState genişletildi:
PENDING
UNKNOWN_RESULT
PaymentAttemptState genişletildi:
PENDING
FAILED
CANCELLED
EXPIRED
UNKNOWN_RESULT
PaymentAttempt üzerine provider/callback reference alanları eklendi:
providerName
providerReference
providerEventId
providerOrderId
...
lastCallbackAt
lastCallbackStatus
Owner command contract eklendi.
Owner command decision helper eklendi.
Deterministic idempotency key helper eklendi:
...
PayTR lookup plan eklendi:
primary: provider_reference
strategy: paytr_merchant_oid_as_provider_reference
Kalan limitler
Worker runtime yoktu.
Payment owner mutation yoktu.
providerReference repository lookup yoktu.
PayTR initiate merchant_oid contract yoktu.
5.9 HARDENING-10C8 — Payment Callback Processing Worker Foundation / No Order Handoff
Durum
PASS WITH LIMITATION
Yapılanlar
processPaymentCallbackRecordsDryRun worker foundation eklendi.
received durumundaki provider callback kayıtları okunmaya başlandı.
normalizedPayload.candidate parse/validate edildi.
decidePaymentCallbackOwnerCommand ile dry-run karar üretildi.
...
non-payment provider → unchanged / skipped
Gerçek record id kullanılarak pending_insert limitation worker seviyesinde telafi edildi.
Payment repository kullanılmadı.
Payment state mutation yapılmadı.
Kalan limitler
...
5.10 HARDENING-10C9-00 — Payment Provider Reference Lookup Foundation / No Owner Mutation
Durum
PASS WITH LIMITATION
Yapılanlar
IPaymentRepository içine provider reference lookup eklendi:
getByProviderReference(
  providerName: string,
  providerReference: string
): Promise<PaymentInitiationResponse | undefined>;
PayTR için karar netleştirildi:
providerReference = merchant_oid
InMemory implementation:
primary: attempt.providerName + attempt.providerReference
fallback: attempt.providerName + attempt.providerEventId
Postgres implementation:
payments.data JSON içinden provider reference lookup
migration eklenmedi
Existing lookup regression korundu:
getByPaymentAttemptId
getByIdempotencyKey
Kalan limitler
providerReference unique constraint yok.
LIMIT 1 kullanılıyor.
initiatePayment henüz providerReference yazmıyordu.
PayTR live initiate yoktu.
Worker owner transition yoktu.
5.11 HARDENING-10C9-01 — Payment Initiation Provider Reference Persistence / No Callback Mutation
Durum
PASS WITH LIMITATION
Yapılanlar
initiatePayment artık provider adapter sonucundaki metadata’yı PaymentAttempt içine yazıyor:
providerName
providerReference
providerEventId
mevcut providerSimulationRef
providerResult.providerReference yoksa crash yok.
Payment state değişmedi:
state=INITIATED
attempt.state=PROVIDER_REDIRECT_READY
Callback alanları yazılmadı:
lastCallbackAt yok
lastCallbackStatus yok
getByProviderReference artık initiate sonrası payment kaydını bulabiliyor.
Existing idempotency regression korundu.
Kalan limitler
PayTR live initiate yok.
Gerçek merchant_oid üretimi yok.
Callback worker owner transition yoktu.
Reconciliation yoktu.
Order handoff yoktu.
5.12 HARDENING-10C9-02 — Payment Owner Transition Succeeded/Failed Callback / No Order Handoff
Durum
PASS WITH LIMITATION
Yapılanlar
İlk kontrollü payment owner mutation eklendi.
Mutation yalnız opt-in worker modunda çalışır:
ownerTransitionMode: 'apply_owner_transition'
Default dry-run davranışı korundu:
ownerTransitionMode: 'dry_run'
Sadece iki command desteklenir:
MARK_PAYMENT_SUCCEEDED
MARK_PAYMENT_FAILED
Lookup sırası:
önce paymentAttemptId
sonra providerReference
MARK_PAYMENT_SUCCEEDED:
payment.state = SUCCEEDED
attempt.state = SUCCEEDED
callbackRecordId, lastCallbackAt, lastCallbackStatus yazılır
MARK_PAYMENT_FAILED:
payment.state = FAILED
attempt.state = FAILED
callbackRecordId, lastCallbackAt, lastCallbackStatus yazılır
Duplicate/idempotent tekrar alreadyApplied=true ile güvenli hale getirildi.
Terminal conflict engellendi:
succeeded → failed geri dönüş yok
failed → succeeded ters geçiş yok
Rejected/signature failed/pending/missing lookup durumları payment mutate etmez.
Order/finance/risk/reconciliation/BFF/migration/env/progress record değişmedi.
Çalıştırılan doğrulamalar
pnpm run typecheck — PASS
...
Kalan limitler
PayTR live initiate yok.
Gerçek PayTR merchant_oid üretimi yok.
Pending/unknown/cancelled/expired owner mutation yok.
Reconciliation runtime yok.
Order handoff yok.
Finance/risk mutation yok.
Real PayTR E2E yok.
6. HARDENING-10 Kapanan Ana Konular

Bu kayıt dönemi sonunda aşağıdaki ana konular kapanmıştır:

Generic provider callback contract foundation.
Provider callback persistence foundation.
Postgres duplicate/idempotency remediation.
Common provider callback BFF ingestion.
Signature/replay/freshness/rate-limit guard foundation.
PayTR callback payload/hash/mapping helper.
PayTR text/plain OK ACK policy.
PayTR callback live BFF normalizedPayload persistence.
Payment provider config/secret resolver.
iyzico secondary-ready config shape.
Payment callback owner command contract.
Payment callback dry-run worker foundation.
providerReference / merchant_oid lookup foundation.
Payment initiation provider reference persistence.
Succeeded/failed callback için opt-in payment owner transition.
7. HARDENING-10 Aktif Kalan Limitler

Aşağıdaki konular hâlâ tamamlanmadı ve sonraki paketlere bırakıldı:

PayTR live initiate yok.
Gerçek PayTR merchant_oid üretimi yok.
Real PayTR sandbox E2E yok.
Reconciliation runtime yok.
PayTR Status Inquiry API entegrasyonu yok.
Pending / unknown / cancelled / expired owner mutation yok.
Order handoff yok.
Payment success event / outbox owner event yok.
Finance / settlement / payout mutation yok.
Risk linkage yok.
Atomic claim/lock yok.
Multi-worker concurrency yok.
Distributed callback worker yok.
providerReference unique constraint/index yok.
Secret manager yok; env foundation seviyesinde.
iyzico callback mapping/implementation yok.
8. Güncel HARDENING-10 Kanıt Seti

Son tamamlanan 10C hattında tekrar tekrar doğrulanan ana komutlar:

pnpm run typecheck
pnpm run build
pnpm --filter @hx/contracts run build
pnpm --filter @hx/payment run build
pnpm --filter @hx/bff run build
pnpm run smoke:provider-boundary
pnpm run smoke:payment-provider-boundary
pnpm run smoke:provider-callback-foundation
pnpm run smoke:provider-callback-postgres
pnpm run smoke:provider-callback-ingestion
pnpm run smoke:provider-callback-signature-guard
pnpm run smoke:provider-callback-replay-guard
pnpm run smoke:provider-callback-freshness-guard
pnpm run smoke:provider-callback-rate-limit-guard
pnpm run smoke:payment-callback-candidate
pnpm run smoke:paytr-callback-mapping
pnpm run smoke:paytr-callback-bff-policy
pnpm run smoke:payment-provider-config
pnpm run smoke:paytr-callback-live-bff-mapping
pnpm run smoke:payment-callback-owner-command
pnpm run smoke:payment-callback-worker-foundation
pnpm run smoke:payment-provider-reference-lookup
pnpm run smoke:payment-initiation-provider-reference
pnpm run smoke:payment-callback-owner-transition

Not:
Her paket kendi closure raporundaki komut setiyle değerlendirilmiştir. Tüm komutlar her pakette aynı anda çalıştırılmış değildir. PASS kararları ilgili paketin kendi closure kanıtlarına dayanır.

9. HARDENING-10 Sonraki Önerilen Paketler

Sıradaki ana paket:

HARDENING-10C10 — Reconciliation / PayTR Status Inquiry Inventory

10C10 doğrudan implementation değil, önce inventory / karar paketi olmalıdır.

10C10’da ele alınacak konular:

shouldReconcile=true callback’lerin lifecycle kararı
PayTR Durum Sorgu API gereklilikleri
pending / unknown_result durumları
paymentAttempt not found durumu
amount/currency mismatch sonrası reconciliation kararı
status inquiry response mapping
retry/backoff ihtiyacı
reconciliation state enum ihtiyacı
order/finance/risk sınırı

Sonraki ana paketler:

HARDENING-10C10 — Reconciliation / PayTR Status Inquiry Inventory
HARDENING-10C11 — Payment Success Event / Order Handoff Boundary
HARDENING-10C12 — Real PayTR Sandbox E2E Validation
10. HARDENING-10 Güncel Nihai Durum
HARDENING-10 — IN PROGRESS / PAYMENT CALLBACK OWNER TRANSITION FOUNDATION COMPLETE
Kapanan seviye

PayTR callback ingestion’dan başlayarak normalizedPayload persistence, worker dry-run, providerReference lookup, payment initiation provider reference persistence ve succeeded/failed callback owner transition seviyesine kadar foundation tamamlanmıştır.

Henüz kapanmayan seviye

HARDENING-10 tamamlanmış sayılmaz. Aşağıdaki ana parçalar bitmeden HARDENING-10 final kapanış verilmemelidir:

Reconciliation / PayTR Status Inquiry
Payment success event / order handoff boundary
Real PayTR sandbox E2E validation
Finance/risk linkage kararları
Production worker/concurrency/claim modeli
Güncel karar
HARDENING-10C9-02 — PASS WITH LIMITATION
Sıradaki önerilen görev — HARDENING-10C10


************************
# HARDENING-10C10 — Final Progress Record Entry
# PayTR Status Inquiry / Payment Reconciliation Line

## Genel Durum

HARDENING-10C10 hattı tamamlandı.

Bu hat, PayTR durum sorgu ve ödeme mutabakatı için güvenli altyapı kurmak amacıyla yürütüldü. Çalışma boyunca provider cevabının doğrudan iş gerçeği kabul edilmemesi, ödeme sonucunun kontrollü doğrulanması, terminal conflict guard’ları, idempotency, audit/outbox evidence ve order handoff ayrımı temel güvenlik kararları olarak korundu.
...
Bu kapanış, order handoff’un açıldığı anlamına gelmez. Payment succeeded hâlâ order created değildir.
...
PayTR status inquiry request/response contract’ları eklendi.
Token/helper ve mapping fonksiyonları eklendi.
Amount/currency mismatch, inconclusive ve failed query ayrıştırıldı.
Provider response doğrudan payment truth yapılmadı.
...
Status inquiry adapter boundary kuruldu.
Internal simulation adapter güvenli envelope dönecek hale getirildi.
Not configured adapter safe rejected/unknown-result davranışı verdi.
Live request bilinçli olarak eklenmedi.

Kapsam dışı:

Gerçek PayTR HTTP çağrısı yok.
Runtime/scheduler yok.
HARDENING-10C10-03 — Reconciliation Decision Contract / Task Model
...
Task candidate modeli eklendi.
Pure decision helper eklendi.
Decision katmanında payment mutation kapalı tutuldu.
...
Reconciliation task persistence/repository foundation eklendi.
...
Ana smoke in-memory repository üzerinden doğrulandı.
Postgres repository build/typecheck seviyesinde doğrulandı.
...
Repository → adapter → decision zinciri çalıştırıldı.
Sadece reconciliation task attempt/status update yapıldı.
...
Amount/currency mismatch command üretmedi.
Inconclusive/failed query command üretmedi.
Manual review durumları command üretmedi.
reconciliation_worker source’u sadece MARK_PAYMENT_SUCCEEDED için kabul edildi.
FAILED → SUCCEEDED engellendi.
CANCELLED → SUCCEEDED engellendi.
SUCCEEDED → FAILED engeli korundu.
...
Worker helper duplication korundu.
Contract/worker equivalence smoke ile kanıtlandı.
...
Controlled payment mutation yalnız explicit opt-in ile açıldı.
Default dry-run davranışı korundu.
Sadece MARK_PAYMENT_SUCCEEDED uygulandı.
Terminal conflict guard korundu.
Duplicate/idempotent davranış doğrulandı.
Explicit opt-in success durumunda payment SUCCEEDED yapılabildi.
...
Mapping → adapter → task → decision → worker → command guard → controlled mutation zinciri uçtan uca doğrulandı.
Explicit opt-in success payment SUCCEEDED yaptı.
Default dry-run state değiştirmedi.
Amount/currency mismatch mutation yapmadı.
Inconclusive/failed query mutation yapmadı.
FAILED/CANCELLED terminal conflict doğrulandı.
Duplicate/idempotency doğrulandı.
Runtime ve source-level no order handoff kontrolü yapıldı.
...
Successful controlled mutation sonrası reconciliation task status reconciled yapılabilir hale getirildi.
Audit evidence eklendi.
Outbox evidence eklendi.
Outbox topic payment.reconciliation.completed olarak sınırlandı.
...
Audit repository tarafında genel idempotent append contract’ı yok.
Deterministic auditId kullanıldı.
Postgres duplicate audit conflict warning’e düşebilir.
10C10 Ana Güvenlik Kararları
PayTR status inquiry response doğrudan business truth kabul edilmedi.
Provider sonucu önce reconciliation candidate’a dönüştürüldü.
Reconciliation önce decision üretir.
Payment mutation yalnız payment owner domain içinde yapılır.
Default worker dry-run kalır.
Payment mutation yalnız explicit opt-in ile çalışır.
Sadece MARK_PAYMENT_SUCCEEDED desteklendi.
FAILED veya CANCELLED payment SUCCEEDED yapılamaz.
Amount/currency mismatch manual review gerektirir.
Inconclusive/failed query payment mutation üretmez.
Payment succeeded order created değildir.
Order handoff 10C10 kapsamı dışında tutuldu.
...
Kalan Limitations
PayTR live/sandbox gerçek request hâlâ yok.
Reconciliation otomatik scheduler/queue ile çalışmıyor.
Controlled mutation explicit fonksiyon üzerinden çalışıyor; production runtime’a bağlanmadı.
Audit evidence var; audit append idempotency Postgres tarafında ayrıca güçlendirilebilir.
Outbox event var; delivery/consumer guarantee yok.
Order handoff hâlâ açılmadı.
Payment succeeded sonrası order üretim kararı 10C11’e bırakıldı.
Nihai Karar
HARDENING-10C10 — CLOSED WITH LIMITATIONS
