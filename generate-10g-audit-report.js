...
const filesToAnalyze = {
  ...
  operationalIntent: [
    'packages/persistence/src/operational-intent.ts',
...
let report = `# PHASE-10G-AUDIT-01-SETTLEMENT-PAYOUT-OPS-REALITY-REPORT.md\n\n`;

report += `## 1. Dosya Varlık Kontrolü\n\n`;
const fileContents = {};
for (const [group, files] of Object.entries(filesToAnalyze)) {
  for (const f of files) {
    const c = read(f);
    fileContents[f] = c;
    report += `- ${c ? 'FOUND' : 'MISSING'} : ${f} - ${c ? 'Mevcut' : 'Bulunamadı'}\n`;
  }
}
...
fs.writeFileSync('PHASE-10G-AUDIT-01-SETTLEMENT-PAYOUT-OPS-REALITY-REPORT.md', report);
