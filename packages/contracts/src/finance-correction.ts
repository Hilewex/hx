export type FinanceCorrectionSourceType =
  | 'PAYMENT'
  | 'ORDER'
  | 'REFUND'
  | 'CANCEL_RETURN'
...
export type FinanceCorrectionReasonCode =
  | 'REFUND_SOURCE_PAYMENT_REFERENCE_MISSING'
  | 'REFUND_AMOUNT_SOURCE_NOT_AVAILABLE'
  | 'SETTLEMENT_IMPACT_PENDING'
  | 'PAYOUT_IMPACT_PENDING'
  | 'PAYMENT_ORDER_RECONCILIATION_REQUIRED'
  | 'CANCEL_RETURN_REFUND_MISMATCH'
  | 'RISK_HOLD_ADVISORY'
  | 'MANUAL_FINANCE_REVIEW'
  | 'UNKNOWN';

export type FinanceCorrectionStatus =
  | 'CREATED'
  | 'UNDER_REVIEW'
  | 'ADVISORY_RECORDED'
  | 'RESOLVED'
...
export type FinanceCorrectionActionType =
  | 'RECORD_ADVISORY'
