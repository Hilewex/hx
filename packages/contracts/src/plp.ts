import { CategoryNode, CategoryFilterDefinition, CategorySortKey, CategorySortOption } from './category';
...
export interface PlpVideoRailItem {
  ...
  name: string;
  mediaType: 'VIDEO';
  /** Store context for PDP navigation and commerce actions */
  storefrontContext?: {
    storefrontId: string;
    name: string;
    slug: string;
  ...
  /** 
   * Boundaries:
   * - This is a commercial product card with video, NOT a story.
   * - supportOnly: true ensures it doesn't leak into narrative story trays.
   * - MiniPDP scope: title, price, variants, add-to-cart capability.
   */
  supportOnly: true;
  discoveryFeed: false;
  warnings?: string[];
