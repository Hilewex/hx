...
exports.PostgresRiskRepository = void 0;
class PostgresRiskRepository {
    ...
    async createSignal(signal) {
        await this.pool.query(`INSERT INTO risk_signals (
    ...
    async createCase(riskCase) {
        await this.pool.query(`INSERT INTO risk_cases (
    ...
    async updateCase(caseId, updates) {
        ...
        if (setClause.length === 0)
            return;
        values.push(caseId);
        await this.pool.query(`UPDATE risk_cases SET ${setClause.join(', ')} WHERE case_id = $${i}`, values);
    }
    ...
    async listCases(query) {
        ...
        const [countResult, rowsResult] = await Promise.all([
            ...
            this.pool.query(`SELECT * FROM risk_cases ${whereString} ORDER BY created_at DESC LIMIT $${i++} OFFSET $${i++}`, [...values, limit, offset]),
        ]);
        return {
    ...
    async listSignals(query) {
        ...
        const [countResult, rowsResult] = await Promise.all([
            ...
            this.pool.query(`SELECT * FROM risk_signals ${whereString} ORDER BY created_at DESC LIMIT $${i++} OFFSET $${i++}`, [...values, limit, offset]),
        ]);
        return {
            total: parseInt(countResult.rows[0].count, 10),
    ...
    async saveIdempotency(key, result) {
        await this.pool.query('INSERT INTO risk_idempotency (idempotency_key, result_id) VALUES ($1, $2) ON CONFLICT DO NOTHING', [key, result]);
    }
    mapToDomain(row) {
        return {
            caseId: row.case_id,
...
exports.PostgresRiskRepository = PostgresRiskRepository;
