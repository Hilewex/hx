import { validateSupplierProtectedAction } from '@hx/supplier-management';
import { SupplierProtectedActionRequest } from '@hx/contracts';

export async function handleSupplierProtectedActionValidate(
  context: any,
  reqBody: any,
  headers: Record<string, string | string[] | undefined>
) {
  try {
    const payload = reqBody as SupplierProtectedActionRequest;
    ...
    if (payload.actorId && actorId && payload.actorId !== actorId) {
      const result = validateSupplierProtectedAction({
        ...payload,
        actorId,
    ...
    const validatedPayload: SupplierProtectedActionRequest = {
      ...payload,
      actorId,
      actorRole
    };

    const result = validateSupplierProtectedAction(validatedPayload);
